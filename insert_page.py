    ...
    def __init__(self, db_cursor, parent=None, customer=common.customer_default):
        ...
        self.widget_insert_page = odict(
            [("fullname",
             ...
             ("internet",
              InsertPageEntryMenu(self.parent, row=4, column=0, label="Internet:", 
                                  options=customer["internet"])),
             ("notes",
              InsertPageEntry(self.parent, row=4, column=1, label="Notes:", text=customer["notes"],
                              callback_map=std_entry_callbacks)),
             ("night_fare",
              InsertPageEntry(self.parent, row=5, column=0, label="Night fare:", text=customer["night_fare"],
                              callback_map=std_entry_callbacks)),
             ("extras",
              InsertPageEntry(self.parent, row=5, column=1, label="Extras:", text=customer["extras"],
                              callback_map=std_entry_callbacks)),
             ("total_price",
              InsertPageEntry(self.parent, row=5, column=2, label="Total price:", text=customer["total_price"],
                              callback_map=std_entry_callbacks)),
             ("payed",
              InsertPageEntry(self.parent, row=6, column=2, label="Payed:", text=customer["payed"],
                              callback_map=std_entry_callbacks)),
             ("balance",
              InsertPageEntry(self.parent, row=7, column=2, label="Balance:", text=customer["balance"],
                              callback_map=std_entry_callbacks, state="disable")),
            ]
        )
